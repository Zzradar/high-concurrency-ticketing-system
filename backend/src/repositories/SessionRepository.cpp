...
void SessionRepository::findById(
    ...
    ErrorCallback onError) const
{
    constexpr const char *sql = R"SQL(
        SELECT session.id, session.event_id,
            TO_CHAR(session.start_time AT TIME ZONE 'Asia/Shanghai', 'MM"月"DD"日"') AS date,
            TO_CHAR(session.start_time AT TIME ZONE 'Asia/Shanghai', 'HH24:MI') AS time,
            CASE EXTRACT(ISODOW FROM session.start_time AT TIME ZONE 'Asia/Shanghai')::INTEGER
                WHEN 1 THEN '周一' WHEN 2 THEN '周二' WHEN 3 THEN '周三'
                WHEN 4 THEN '周四' WHEN 5 THEN '周五' WHEN 6 THEN '周六'
                WHEN 7 THEN '周日' END AS weekday,
            venue.name || ' · ' || session.hall_name AS venue,
            TO_CHAR(session.gate_time AT TIME ZONE 'Asia/Shanghai', 'HH24:MI') AS gate_time,
            session.status AS database_status,
            COALESCE(MIN(inventory.price), 0)::BIGINT AS price_from,
            COUNT(inventory.id)::BIGINT AS total_count,
    ...
    drogon::app().getDbClient("default")->execSqlAsync(
        ...
        [onSuccess = std::move(onSuccess)](const drogon::orm::Result &rows) {
            ...
            onSuccess(SessionRow{
                ...
                .venue = row["venue"].as<std::string>(),
                .gateTime = row["gate_time"].as<std::string>(),
                .databaseStatus = row["database_status"].as<std::string>(),
                .priceFrom = row["price_from"].as<std::int64_t>(),
                .totalCount = row["total_count"].as<std::int64_t>(),
        ...
        [onError = std::move(onError)](const drogon::orm::DrogonDbException &error) {
            LOG_ERROR << "Failed to find session: " << error.base().what();
            onError();
        },
        sessionId);
}
