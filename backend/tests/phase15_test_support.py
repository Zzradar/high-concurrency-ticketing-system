...
def sql(statement,ok=True):
    r=subprocess.run(['docker','exec','-i','phase15-api-postgres','psql','-U','postgres','-qAt','-v','ON_ERROR_STOP=1'],input=statement,capture_output=True,text=True,encoding='utf-8')
...
def redis(*args):
    r=subprocess.run(['docker','exec','phase15-redis','redis-cli','--json',*map(str,args)],capture_output=True,text=True,encoding='utf-8')
