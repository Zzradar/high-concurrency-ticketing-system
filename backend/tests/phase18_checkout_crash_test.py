...
class CheckoutCrash(unittest.TestCase):
 def test_committed_order_recovers_after_crash_and_sales_end(self):
  ...
  name=topology()['prefix']+'-checkout-crash-'+uuid.uuid4().hex[:8]
  def docker(*args):
  ...
  fixture.window('ENDED');auth.BASE_URL=os.environ['PHASE18_BASE_URL'];docker('start',topology()['api']);until(healthy,seconds=20)
