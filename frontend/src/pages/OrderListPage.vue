...
<template>
  <main class="page-shell">
    <PageBreadcrumbs :items="[{ label: '我的订单' }]" />
    ...
    <PageState v-if="error" eyebrow="ORDERS" title="订单加载失败" :description="error" action-label="重新加载" @action="load" />
    <div v-else-if="loading" class="session-list" aria-label="正在加载订单">
      <div v-for="index in 3" :key="index" class="session-card skeleton-card"></div>
    </div>
    <PageState v-else-if="!orders.length" eyebrow="ORDERS" title="暂无订单" description="完成一次选座和确认后，订单会出现在这里。">
      <RouterLink class="secondary-button" :to="{ name: routeNames.events }">返回活动列表</RouterLink>
    </PageState>
    <section v-else class="session-list">
      <button v-for="order in orders" :key="order.id" class="session-card" type="button" @click="router.push({ name: routeNames.orderDetail, params: { orderId: order.id } })">
