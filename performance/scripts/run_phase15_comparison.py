...
Both images must be built from the recorded merged baseline and Phase15 binary.
...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('variant',choices=['baseline','current']);args=parser.parse_args()
    phase14.GENERATED=ROOT/'performance/generated/phase15-comparison'
    targets=load_targets(smoke=True)
    targets['environment']['localPort']=18097
    targets['environment']['prometheusPort']=19097
    project='phase14-phase15-'+args.variant
    override=ROOT/'backend/build/phase15'/('compose-'+args.variant+'.json')
    override.write_text(json.dumps({'services':{'backend':{'image':'phase15-'+args.variant+':local','healthcheck':{'test':['CMD','python3','-c',"import json,urllib.request;assert json.load(urllib.request.urlopen('http://127.0.0.1:8080/health'))=={'database':'up','status':'ok'}"]}}}}),encoding='utf-8')
    ...
    prepare=Environment(targets,phase14.RESULTS/('phase15-'+args.variant+'-prepare'),project=project)
    if not (prepare.data/'base.dump').exists():prepare.prepare()
    records=[]
    try:
        for case in ['G0','U1','U2']:
            env=Environment(targets,phase14.RESULTS/('phase15-'+args.variant+'-'+case.lower()),project=project)
        ...
        output=ROOT/'performance/experiments/phase15-sales-window';output.mkdir(parents=True,exist_ok=True)
