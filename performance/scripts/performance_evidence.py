...
PROMETHEUS_QUERIES = {
    ...
    "password-hash-queue-depth": "ticketing_password_hash_queue_depth",
    "password-hash-active-workers": "ticketing_password_hash_active_workers",
    "password-hash-submissions": "sum(increase(ticketing_password_hash_submissions_total[1m])) by (outcome)",
    "password-hash-queue-wait-p50": "histogram_quantile(0.50,sum(rate(ticketing_password_hash_queue_wait_seconds_bucket[1m])) by (le))",
    "password-hash-queue-wait-p95": "histogram_quantile(0.95,sum(rate(ticketing_password_hash_queue_wait_seconds_bucket[1m])) by (le))",
    "password-hash-queue-wait-p99": "histogram_quantile(0.99,sum(rate(ticketing_password_hash_queue_wait_seconds_bucket[1m])) by (le))",
    "password-hash-execution-p50": "histogram_quantile(0.50,sum(rate(ticketing_password_hash_execution_seconds_bucket[1m])) by (le))",
    "password-hash-execution-p95": "histogram_quantile(0.95,sum(rate(ticketing_password_hash_execution_seconds_bucket[1m])) by (le))",
    "password-hash-execution-p99": "histogram_quantile(0.99,sum(rate(ticketing_password_hash_execution_seconds_bucket[1m])) by (le))",
