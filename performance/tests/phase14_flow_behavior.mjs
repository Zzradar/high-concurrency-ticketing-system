...
async function harness({fn='burst',index=0,failConfirm=false,failLogin=false,neverPay=false,kind='J1',role='main'}={}) {
    let clock=100000,jarUser=null;const requests=[],points=[],sleeps=[];
    ...
    const http={request:respond,batch:items=>items.map(x=>respond(x.method,x.url,null,x.params)),cookieJar:()=>({clear:()=>{jarUser=null;}})};
    ...
    const flow=await link('./phase14-flow.js');await flow.evaluate();flow.namespace[fn]();
    if(fn==='online'){execution.vu.idInTest=999;flow.namespace.online();}
    ...
    return {requests,points,sleeps,options:flow.namespace.options};
}
