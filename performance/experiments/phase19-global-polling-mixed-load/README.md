...
当前处于 Stage0/协议准备阶段，尚未完成 Phase19，尚未修改前端生产轮询逻辑，尚未开始正式 SUT 容量测试或浏览器 A/B，尚未冻结完整正式 A/B 协议。
...
`protocol/` 中的独立游标与混合流程脚本仍在准备，生成器实际使用的五个文件哈希逐点记录并校验一致。后续变更执行协议必须重新资格/冻结，不把不同协议的结果合并为正式 A/B。

浏览器全局轮询 A/B、SUT actual VU、混合读写、传播延迟、5000 累计旅程、1000 人热点以及前端实现仍未交付。这里没有生产 SLA、Stripe 容量或最大吞吐结论。

正式协议完成并冻结前，不得把本目录的 Stage0 检查点当作 Phase19 完成报告或独立复验交付。
