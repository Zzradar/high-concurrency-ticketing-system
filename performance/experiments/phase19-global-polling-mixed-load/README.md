...
Stage0 完成。r1 正式基线因 L2 无效已整组保留至 `diagnostics/r1-formal-retired/`，当前 r2 协议正在重新资格，尚未冻结。尚未完成 Phase19，尚未修改前端生产轮询逻辑。
...
`protocol/` 是待冻结 r2；重新资格使用完整文件哈希，正式运行将逐点校验。正式运行拒绝协议漂移、未资格的资源身份或超出资格的 VU；后续变更协议必须重新冻结并整组重跑正式基线，不能拼接 A/B。
