...
def main():
    ...
    save(out/'identity.json',{'startedUtc':datetime.now(timezone.utc).isoformat(),'diagnostic':args.diagnostic,
        ...
        'rawPoints':'private point output/k6-points.jsonl.gz; native lossless k6 gzip; no auth headers, bodies or high-cardinality identifiers in metric tags'})
    run('docker','run','-d','--name',name,'--label','phase19.owner='+stack.prefix,'--network',stack.prefix+'-data',
        ...
        stack.config['images']['k6']['id'],'run','--quiet','--out','json=/output/k6-points.jsonl.gz','/protocol/workload.js')
    ...
    points=out/'k6-points.jsonl.gz'
    aggregated=aggregate(points) if points.exists() else None
    if aggregated:save(out/'aggregate.json',aggregated)
    result={'metricSummaryAvailable':bool(summary),'rawPointsFormat':'gzip JSONL; native k6 lossless export',
        'valid':bool(summary and resources and exit_code==0 and not reason and not observer_errors and after['passed'] and count('phase19_errors')==0 and count('dropped_iterations')==0 and count('phase19_started')==count('phase19_completed')),
    ...
    if not summary:
        for key in ['startedIterations','completedBusinessIterations','k6CompletedIterations','interruptedIterations','droppedIterations','httpRequestCount','cursorAdvances','initializedActiveVUs']:
            result[key]=None
