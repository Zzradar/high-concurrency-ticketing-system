...
def accounting(aggregate,mode,duration,read_rate,transition_rate,journey_rate):
    ...
    planned={}
    if mode=='open':
        planned={'readerIterationsNominal':duration*read_rate,'writerFlowsNominal':duration*transition_rate*5/14}
        for i in range(10):
            n=scenarios.get('readers'+str(i),0)
            if not duration*read_rate/10<=n<=duration*read_rate/10+1:errors.append('reader arrival stream '+str(i)+' differs from plan')
...
def main():
    ...
    if aggregated:
        result['accounting']=accounting(aggregated,args.mode,duration,args.read_rate,args.transitions_rate,args.journey_rate)
