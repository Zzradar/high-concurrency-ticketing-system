...
const scenarios = mode === 'open' ? {
  ...Object.fromEntries(Array.from({ length: 10 }, (_, i) => [`readers${i}`, { executor: 'constant-arrival-rate', exec: 'openReader', startTime: `${i * 1000 / rate}ms`, rate: rate / 10, timeUnit: '1s', duration: `${duration}s`, preAllocatedVUs: readerVus / 10, maxVUs: readerVus / 10, gracefulStop: '40s' }])),
