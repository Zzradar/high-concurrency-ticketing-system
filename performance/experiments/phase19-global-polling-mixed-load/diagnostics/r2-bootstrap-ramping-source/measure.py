...
def accounting(aggregate,mode,duration,read_rate,transition_rate,journey_rate,warmup=0):
    ...
    planned={}
    if mode=='open':
        planned={'readerIterationsNominal':(duration-warmup)*read_rate+warmup*min(50,read_rate),'writerFlowsNominal':duration*transition_rate*5/14}
        for i in range(10):
...
def main():
    ...
    save(out/'identity.json',{'startedUtc':datetime.now(timezone.utc).isoformat(),'diagnostic':args.diagnostic,
        ...
        'plannedReadIterationsTotal':args.seconds*args.read_rate+args.warmup*min(50,args.read_rate) if args.mode=='open' else None,
        'warmupDeltaIterationsPerSecond':min(50,args.read_rate) if args.mode=='open' else None,
    ...
    if aggregated:
        result['accounting']=accounting(aggregated,args.mode,duration,args.read_rate,args.transitions_rate,args.journey_rate,args.warmup)
