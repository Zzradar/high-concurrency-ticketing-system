...
function readerScenario(i) {
  const common = { exec: 'openReader', startTime: `${i * 1000 / rate}ms`, timeUnit: '1s', preAllocatedVUs: readerVus / 10, maxVUs: readerVus / 10, gracefulStop: '40s' };
  return warmup > 0 ? { ...common, executor: 'ramping-arrival-rate', startRate: Math.min(50, rate) / 10,
    stages: [{ duration: `${warmup}s`, target: Math.min(50, rate) / 10 }, { duration: '0s', target: rate / 10 }, { duration: `${duration - warmup}s`, target: rate / 10 }] }
    : { ...common, executor: 'constant-arrival-rate', rate: rate / 10, duration: `${duration}s` };
}
