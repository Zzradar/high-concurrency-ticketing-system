...
def stop_reason(samples):
    ...
    if len(samples) >= 3:
        a, b, c = samples[-3:]
        limit = last["memoryLimit"]
        first, second = b["memory"] - a["memory"], c["memory"] - b["memory"]
        if all(s.get("memoryLimit") == limit for s in (a, b, c)) and first >= .05 * limit and second >= first:
            return "memory grows >= 5% of limit across three samples without deceleration"
    return None
