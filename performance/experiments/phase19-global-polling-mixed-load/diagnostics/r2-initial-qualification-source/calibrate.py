...
def main():
    ...
    result.update(endedUtc=datetime.now(timezone.utc).isoformat(),rawPointsSha256=hashlib.sha256(points.read_bytes()).hexdigest() if points.exists() else None,
                  rawPointsBytes=points.stat().st_size if points.exists() else None)
