# Phase19 protocol candidate (freeze required before formal runs)
...
bodies and Delta parsing. Each VU idles 15 seconds before work. Qualify 100, 250,
...
90% for three samples stop a point. Sampling includes RSS/HWM, cgroup memory,
...
| Open L1 | read 100, transitions 10, seconds 60 | before L2 |
| Open L2 | read 250, transitions 25, seconds 120 | before L3 |
| Open L3 | read 500, transitions 50, seconds 120 | stop escalation at first invalid/unsafe point |
...
Open uses ten independently paced arrival streams and a distinct writer scenario.
Set reader/writer preallocation explicitly (defaults 100/100, fixed maximum);
never lower the requested arrival target to relabel a failed tier. First actual
reader use establishes its own Snapshot, then each scheduled iteration performs
Delta. Initial snapshots and hasMore catch-up are extra HTTP requests and are
reported separately from target Delta iterations. k6 scheduling at the duration
boundary can create an additional iteration per arrival stream; retain nominal
planned, actual started, business completed, k6 completed, interrupted and dropped.
...
is assumed. Raw k6 JSONL and browser profiles remain private; retain SHA256 and byte
