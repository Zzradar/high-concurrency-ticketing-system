...
try{
 ...
 for(const [state,target,hidden,duration]of [['visible',seat,false,10000],['hidden',other,true,91000],['restored',seat,false,10000]]){
 ...
 const hidden=result.phases.find(x=>x.state==='hidden');const restored=result.phases.find(x=>x.state==='restored');
 assert.equal(result.maxInflight,1);
 assert(restored.actionAt-hidden.observedAt>=90000,'Three maximum 30-second polling cycles must elapse');
 assert.equal(result.requests.filter(r=>r.startEpochMs>=hidden.observedAt && r.startEpochMs<restored.actionAt).length,0);
 const resumed=result.requests.filter(r=>r.startEpochMs>=restored.actionAt);
 assert.equal(resumed.filter(r=>r.mode==='snapshot').length,1,'Exactly one authoritative restoration Snapshot');
 assert(resumed[0].startEpochMs-restored.actionAt<1000,'Restoration must read immediately');
 result.purpose='Additional long-hidden capability gate; NOT frozen-protocol A/B';
 result.passed=true;
