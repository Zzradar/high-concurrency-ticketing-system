...
out=Path(os.environ['TEMP'])/'phase19-avfix-full-regression';out.mkdir(exist_ok=True)
prefix='phase18-phase19-avfix-full';network='phase19-avfix-full-regression-data'
...
run('docker','network','create','phase19-avfix-full-regression-ingress')
...
config['custom_config']['authentication']['allowed_origins']+=['http://127.0.0.1:18450']
(out/'config.json').write_text(json.dumps(config))
print('Fresh Phase19 regression database and Redis prepared.',flush=True)
(out/'ticketing_backend').write_bytes((Path(os.environ['TEMP'])/'phase19-avfix-binary/ticketing_backend').read_bytes())
env=dict(os.environ,TICKETING_ADMISSION_HMAC_SECRET=secrets.token_hex(32))
args=['docker','run','-d','--name',prefix+'-api','--label','phase19.owner=stage0','--network',network,'--cpus','2','--memory','1g','--pids-limit','256','--ulimit','nofile=4096:4096','-p','127.0.0.1:18442:8080','--mount',f'type=bind,source={out},target=/stage0,readonly','-e','TICKETING_ADMISSION_HMAC_SECRET','--entrypoint','/stage0/ticketing_backend','phase14-engineering-build:20260910-v3','/stage0/config.json']
p=subprocess.run(args,env=env,capture_output=True,text=True)
if p.returncode:raise RuntimeError(p.stderr)
run('docker','network','connect','phase19-avfix-full-regression-ingress',prefix+'-api')
import urllib.request
opener=urllib.request.build_opener(urllib.request.ProxyHandler({}))
for i in range(100):
 try:
  if opener.open('http://127.0.0.1:18442/health',timeout=2).status==200:break
