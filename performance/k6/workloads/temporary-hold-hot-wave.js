...
const config = loadConfig('temporary-hold-hot-wave');
...
if (config.mode !== 'wave' || ![100, 500, 1000].includes(contenders)) {
    throw new Error('temporary hold hot wave requires MODE=wave and 100/500/1000 contenders');
}
if (sessions.length < contenders) throw new Error('temporary hold wave Session pool exhausted');
...
export function wave(state) {
    ...
    waveStartOffset.add(Date.now() - state.releaseAt, {step: 'temporary_hold_wave'});
    const index = exec.scenario.iterationInTest;
    if (index >= contenders || index >= sessions.length) throw new Error(`temporary hold wave pool exhausted at ${index}`);
    const response = http.post(
        `${config.baseUrl}/checkout-sessions`,
        JSON.stringify({sessionId: dataset.hotSessionId, seatIds: [dataset.hotSessionSeatId]}),
        {headers: mutationHeaders(sessions[index]), tags: {name: 'POST /checkout-sessions'}},
    );
    let result = 'unexpected';
    if (response.status === 201 && response.json('id')) {
        result = 'success'; waveSuccess.add(1, {step: 'temporary_hold_wave'});
    } else if (response.status === 409 && response.json('code') === 'SEAT_TEMPORARILY_HELD') {
        result = 'business_conflict'; waveConflict.add(1, {step: 'temporary_hold_wave'});
    } else if (response.status === 0 || response.status >= 500) result = 'system_error';
    recordResult(result, 'temporary_hold_wave');
}
